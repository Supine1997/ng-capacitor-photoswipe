    ...
    var NgCapacitorPhotoswipeDirective = /** @class */ (function () {
        function NgCapacitorPhotoswipeDirective(el, capacitorPhotoswipeService) {
            ...
            el.nativeElement.addEventListener('click', function () {
                // attr上的值
                var attrValue = el.nativeElement.getAttribute('libPhotoswipe');
                if (attrValue) { // 如果存在值，则表示为群组显示
                    var imagesNodeArray = document.body.querySelectorAll("img[libPhotoswipe=" + attrValue + "]:not(.pswp__img)");
                    var index = _this.getNodeIndexFromList(el.nativeElement, imagesNodeArray);
                    _this.capacitorPhotoswipeService.showFromElements(index, imagesNodeArray);
                }
                else {
                    _this.capacitorPhotoswipeService.showFromElement(el.nativeElement);
                }
            });
        }
        /**
